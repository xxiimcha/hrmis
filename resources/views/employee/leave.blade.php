                                    ...
                                    <tbody>
                                        @foreach ($leaveRequests as $lr)
                                            <tr scope="row" data-mdb-index="0">
                                                <td data-mdb-field="field_1"><div style="padding-top: 5px;">{{ $lr->dateoffiling }}</div></td>
                                                <td data-mdb-field="field_2">
                                                    <div style="padding-top: 5px;" class="
                                                        @if($lr->status == 'Pending') status-pending
                                                        @elseif($lr->status == 'Approved') status-approved
                                                        @elseif($lr->status == 'Rejected') status-rejected
                                                        @elseif($lr->status == 'Cancelled') status-cancelled
                                                        @endif">
                                                        {{ $lr->status }}
                                                    </div>
                                                </td>
                                                <td data-mdb-field="field_3">
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <a role="button" href="/welcome/employee/leave/list/remove/{{ $lr->id }}" class="btn btn-sm btn-danger bluish_button d-flex align-items-center cancel-button @if ($lr->status !== 'Pending') sr-only @endif">
